...
class OperationCameraConfig:
    ...
    camera_poses: Mapping[str, CameraPose] = field(default_factory=dict)
...
def _generate_operation_camera_specs(
    ...
    return [
        CameraSpec(
            name=spec.name,
            pos=np.asarray(config.camera_poses[spec.name].position, dtype=np.float64),
            quat=np.asarray(
                config.camera_poses[spec.name].quaternion_wxyz,
                dtype=np.float64,
            ),
            fovy=spec.fovy,
        )
        if spec.name in config.camera_poses
        else spec
        for spec in specs
    ]
